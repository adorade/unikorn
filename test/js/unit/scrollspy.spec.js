/**
 * UniKorn (v1.1.0): scrollspy.spec.js
...
$(() => {
  ...
  module('scrollspy', {
    beforeEach: () => {
      ...
      $.fn.unikornScrollspy = $.fn.scrollspy.noConflict()
    },
    afterEach: () => {
      $.fn.scrollspy = $.fn.unikornScrollspy
      delete $.fn.unikornScrollspy
      $('#qunit-fixture').html('')
    }
  })
  ...
  test('should only switch "active" class on current target', (assert) => {
    assert.expect(1)
    ...
    var $scrollspy = $section
      ...
      .unikornScrollspy({ target: '#ss-target' })
  ...
  test('should only switch "active" class on current target specified w element', (assert) => {
    assert.expect(1)
    var done = assert.async()
  ...
  test('should correctly select middle navigation option when large offset is used', (assert) => {
    assert.expect(3)
    var done = assert.async()
    ...
    $scrollspy.unikornScrollspy({ target: '#navigation', offset: $scrollspy.position().top })
  ...
  test('should add the active class to the correct element (nav markup)', (assert) => {
    assert.expect(2)
    var done = assert.async()
  ...
  test('should add the active class to the correct element (list-group markup)', (assert) => {
    ...
    var $content = $(contentHtml)
      .appendTo('#qunit-fixture')
      .unikornScrollspy({ offset: 0, target: '.navbar' })
  ...
  test('should add the active class correctly when there are nested elements at 0 scroll offset', (assert) => {
    assert.expect(6)
    var times = 0
    var done = assert.async()
  ...
  test('should add the active class correctly when there are nested elements (nav markup)', (assert) => {
    assert.expect(6)
    var done = assert.async()
    var times = 0
  ...
  test('should add the active class correctly when there are nested elements (nav nav-item markup)', (assert) => {
    assert.expect(6)
    var done = assert.async()
    var times = 0
  ...
  test('should add the active class correctly when there are nested elements (list-group markup)', (assert) => {
    assert.expect(6)
    var times = 0
    var done = assert.async()
  ...
  test('should clear selection if above the first section', (assert) => {
    assert.expect(3)
    var done = assert.async()
  ...
  test('should NOT clear selection if above the first section and first section is at the top', (assert) => {
    assert.expect(4)
    var done = assert.async()
  ...
  test('should correctly select navigation element on backward scrolling when each target section height is 100%', (assert) => {
    assert.expect(5)
    var done = assert.async()
  ...
  test('should allow passed in option offset method: offset', (assert) => {
    assert.expect(4)

    var testOffsetMethod = function (type) {
      var $navbar = $('<nav class="navbar"' + (type === 'data' ? ' id="navbar-offset-method-menu"' : '') + '>' +
          '<ul class="nav">' +
      ...
      var $content = $('<div class="content"' + (type === 'data' ? ' data-spy="scroll" data-target="#navbar-offset-method-menu" data-offset="0" data-method="offset"' : '') + ' style="position: relative; overflow: auto; height: 100px">' +
      ...
      if (type === 'js') {
        $content.unikornScrollspy({ target: '.navbar', offset: 0, method: 'offset' })
      } else if (type === 'data') {
        $(window).trigger('load')
      }

      var $target = $('#div-' + type + 'm-2')
      var scrollspy = $content.data('uni.scrollspy')
  ...
  test('should allow passed in option offset method: position', (assert) => {
    assert.expect(4)
    ...
    var testOffsetMethod = function (type) {
      ...
      var $content = $('<div class="content"' + (type === 'data' ? ' data-spy="scroll" data-target="#navbar-offset-method-menu" data-offset="0" data-method="position"' : '') + ' style="position: relative; overflow: auto; height: 100px">' +
          '<div id="div-' + type + 'm-1" style="position: relative; height: 200px; padding: 0; margin: 0">div 1</div>' +
          '<div id="div-' + type + 'm-2" style="position: relative; height: 150px; padding: 0; margin: 0">div 2</div>' +
          '<div id="div-' + type + 'm-3" style="position: relative; height: 250px; padding: 0; margin: 0">div 3</div>' +
          '</div>'
